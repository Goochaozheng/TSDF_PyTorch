# Modified from Andy Zeng's tsdf-fusion-python: https://github.com/andyzeng/tsdf-fusion-python
...
class TSDFVolume:
  ...
  def extractMesh(self):
    """
    Compute a mesh from the voxel volume using marching cubes using open3D.
    Done in async mode
    """

    def doMeshing():
      tsdf_vol, _ = self.get_volume()
      # vertices, faces, norms, vals = measure.marching_cubes_lewiner(tsdf_vol, level=0)
      vertices, triangles = mcubes.marching_cubes(tsdf_vol, 0)

      vertices = vertices*self._voxel_size+self._vol_origin
      
      # offset = vertices.shape[0] % 3
      # vertices = vertices[:-offset, :]
      # triangles = np.arange(vertices.shape[0]).reshape((-1, 3))

      o3d_mesh = o3d.geometry.TriangleMesh()
      o3d_mesh.vertices = o3d.utility.Vector3dVector(vertices.astype(float))
      o3d_mesh.triangles = o3d.utility.Vector3iVector(triangles.astype(np.int32))
    ...
    # self.meshing_thread = threading.Thread(target=doMeshing)
