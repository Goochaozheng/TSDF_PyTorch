...
class TSDFVolume:
    ...
    def integrate(self, depth_img, intrinsic, cam_pose, weight: float=1.):
        ...
        # TODO: valid_vox_x, valid_vox_y, valid_vox_z = self._vox_coords[voxel_mask, :]
    ...
    def update_tsdf(self, tsdf_old, tsdf_new, weight_old, obs_weight):
        ...
        #     weight_new[i] = weight_old[i] + obs_weight
        #     tsdf_vol_int[i] = (weight_old[i] * tsdf_old[i] + obs_weight * tsdf_new[i]) / weight_new[i]

        # TODO: Using torch matrix operation instead of loop
